...
class EmailFlowController(
    ...
    suspend fun start(email: String) = exclusive {
        Log.d("EmailDebugX", "Requesting verification email")
        ...
        try {
            ...
            Log.d("EmailDebugX", "Verification email requested; waiting for link")
        } catch (cancelled: CancellationException) {
            throw cancelled
        } catch (error: Exception) {
            Log.e("EmailDebugX", "Email request failed", error)
    ...
    suspend fun openLink(link: VerificationLink) = exclusive {
        ...
        Log.d("EmailDebugX", "Opened verification link request_id=${link.requestId} local_proof=${pending != null} valid_format=${link.isValid()}")
        if (!link.isValid()) {
            Log.w("EmailDebugX", "Verification link has invalid proof format")
            screen = EmailScreen.Unusable(email, link.requestId.takeIf { it.matches(Regex("[A-Za-z0-9_-]{43}")) })
            return@exclusive
        }
        if (pending == null) {
            Log.d("EmailDebugX", "Verification link opened without matching local proof; requesting manual code")
            screen = EmailScreen.Code(email, link)
            return@exclusive
        }
        Log.d("EmailDebugX", "Verification link matched local proof; confirming automatically")
        ...
        try {
            ...
            Log.e("EmailDebugX", "Automatic confirmation rejected request_id=${link.requestId} kind=${error.kind}", error)
            ...
            Log.e("EmailDebugX", "Automatic confirmation failed request_id=${link.requestId}", error)
    ...
    suspend fun confirmCode(code: String) = exclusive {
        ...
        if (!code.matches(Regex("[0-9]{6}"))) {
            Log.d("EmailDebugX", "Manual code rejected by local format validation")
            screen = current.copy(error = "Enter the six-digit code in your email.")
            return@exclusive
        }
        Log.d("EmailDebugX", "Submitting manual confirmation code")
        screen = EmailScreen.Loading(LoadingStep.CONFIRM_EMAIL)
        try {
            ...
            Log.e("EmailDebugX", "Manual confirmation rejected request_id=${current.link.requestId} kind=${error.kind}", error)
            screen = when (error.kind) {
            ...
            Log.e("EmailDebugX", "Manual confirmation failed", error)
    ...
    suspend fun resend() = exclusive {
        ...
        if (id == null || !canResend()) {
            Log.d("EmailDebugX", "Resend skipped: request missing or cooldown active")
            return@exclusive
        }
        Log.d("EmailDebugX", "Requesting another verification email")
        ...
        try {
            ...
            Log.d("EmailDebugX", "Verification email resent")
        } catch (cancelled: CancellationException) {
            throw cancelled
        } catch (error: Exception) {
            Log.e("EmailDebugX", "Resend failed", error)
            screen = EmailScreen.Wait(email, id, "We couldn't request another email. Please try again.")
        }
    }

    fun changeEmail() {
        Log.d("EmailDebugX", "Clearing pending email and returning to start")
    ...
    private fun complete(requestId: String, session: AuthSession) {
        try {
            ...
            Log.d("EmailDebugX", "Email verified; session stored")
        } catch (error: Exception) {
            Log.e("EmailDebugX", "Email confirmed but session handling failed request_id=$requestId", error)
    ...
    private suspend inline fun exclusive(crossinline action: suspend () -> Unit) {
        if (!requestLock.tryLock()) {
            Log.d("EmailDebugX", "Ignoring action while another email request is running")
            return
        }
